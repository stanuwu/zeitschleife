defmodule LoopScanner do
    def is_loop(data, series_length) do
        Enum.any?(0..length(data) - 1, fn i ->
            Enum.any?(0..length(data) - 1, fn j ->
                scan_recursive(data, i, j, series_length)
            end)
        end)
    end

    def scan_recursive(data, pattern_index, data_index, scan_length) do
        # Base case: If scan_length is 0, return true
        if scan_length == 0 do
            true
        else
            # Base case: If indices are out of bounds
            if pattern_index >= length(data) or data_index >= length(data) do
                false
            else
                # Base case: If pattern_index and data_index are the same, return false
                if pattern_index == data_index do
                    false
                else
                    # If the values at pattern_index and data_index don't match, return false
                    if Enum.at(data, pattern_index) != Enum.at(data, data_index) do
                        false
                    else
                        # Recursively check the next pair
                        scan_recursive(data, pattern_index + 1, data_index + 1, scan_length - 1)
                    end
                end
            end
        end
