...
sub scan_recursive{
    my ($pattern_index, $data_index, $scan_length) = @_;
    
    if($scan_length == 0){
        return 1;
    }
    if($pattern_index >= @data || $data_index >= @data){
        return 0;
    }
    ...
    return scan_recursive($pattern_index + 1, $data_index + 1, $scan_length - 1);
}