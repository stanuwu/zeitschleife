...
def is_loop(data, length)
  i = 0
  l = 0
  until i >= 1000
    until l >= 1000
      if scan_recursive(data, i, l, length)
        return true
      end
      l += 1
    end
...
def scan_recursive(data, pattern_index, data_index, scan_length)
  if scan_length == 0
    return true
  end
  if pattern_index >= data.size || data_index >= data.size
    return false
  end
  if pattern_index == data_index
    return false
  end
  if data[pattern_index] != data[data_index]
    return false
  end
  return scan_recursive(data, pattern_index + 1, data_index + 1, scan_length - 1)
end

random_data(data)
if is_loop(data, SERIES_LENGTH)
  puts "Time Loop Found!"
else
  puts "No Time Loop Found!"
end
